	...
	if len(emptyKeys) != 0 {
		...
		if castError != nil {
			...
			if castError != nil {
				...
				generalEvents, err := app.EventRepository.FindByJalaliMonth(monthInt, yearInt, model.EventTypeGeneral, "")

				var personalEvents []model.Event

				if session != nil && session.User != nil && session.User.ID != "" {
					personalEvents, err = app.EventRepository.FindByJalaliMonth(monthInt, yearInt, model.EventTypePersonal, session.User.ID)
				}

				panelEvents, err := app.EventRepository.FindByJalaliMonth(monthInt, yearInt, model.EventTypePanel, "")

				if err != nil {
					responseStatus = ResponseStatusError
					data["err_code"] = 0
					data["err_text"] = "get events error"
				} else {
