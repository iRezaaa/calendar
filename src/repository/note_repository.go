...
func (r *NoteRepository) FindByID(objectID bson.ObjectId) (model.Note, error) {
	var note model.Note
...
func (r *NoteRepository) UpdateOrInsert(note model.Note) (*mgo.ChangeInfo , error) {
	changeInfo, err := r.DB.C("notes").Upsert(bson.M{
		"day":     note.Day,
		"month":   note.Month,
		"year":    note.Year,
		"user_id": note.UserID,
